 ...
 */
#ifndef MBED_SPIF_BLOCK_DEVICE_PD_H
#define MBED_SPIF_BLOCK_DEVICE_PD_H

//library is effectively identical to normal SPIFBlockDevice library
//it just powers up and down the flash memory on the init() and deinit() calls respectively
 ...
 *  SPIFBlockDevicePD spif(PTE2, PTE4, PTE1, PTE5);
 *
 *  int main() {
 *      printf("spif test\n");
 *
 ...
 *  @endcode
 */
class SPIFBlockDevicePD : public mbed::BlockDevice {
public:
    /** Creates a SPIFBlockDevicePD on a SPI bus specified by pins
     *
     ...
     */
    SPIFBlockDevicePD(PinName mosi, PinName miso, PinName sclk, PinName csel, int freq = 40000000);
